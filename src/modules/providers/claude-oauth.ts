...
async function postJson(
  ...
  // A vendor can hand back a usable credential on a non-200 — Anthropic signs
  // the account in but flags the plan over its usage limit with a 429. If the
  // body carries real tokens, the sign-in succeeded; only fall through to error
  ...
  if (!res.ok) {
    ...
    // its usage limit (5h rolling / weekly) — not a failed sign-in, so it gets
    // its own message instead of a bare "sign-in failed: 429".
    if (res.status === 429) {

...
/**
 * OAuth sign-in for a ChatGPT subscription (Plus/Pro). Regentry opens the
 * auth.openai.com approval page in a new tab, captures the localhost redirect,
 * and exchanges the code — the only thing the user must do is approve on that
 * page.
 *
...
export function ChatGPTSignIn({
  ...
  const start = useCallback(async () => {
    ...
    try {
      const credential = await signInWithChatGPT(controller.signal, (authorizeUrl) =>
    ...
    return (
      <div className="flex flex-col gap-2 rounded-lg border border-brand-200 bg-brand-50 p-3 dark:border-brand-800 dark:bg-brand-950">
        <p className="text-xs text-neutral-600 dark:text-neutral-300">
          {t("providerForm.chatgptSignInOpened")}
        </p>
        <p className="text-xs text-neutral-500 dark:text-neutral-400">
          {t("providerForm.chatgptSignInBlocked")}{" "}
          <a
      ...
      >
        <span className="text-sm font-medium text-amber-900 dark:text-amber-100">
          {phase.step === "error"
            ? phase.message
            : expired
              ? t("providerForm.chatgptSignInExpiredTitle")
              : t("providerForm.signInDeniedTitle")}
        </span>
        {phase.step === "failed" && (
          <span className="text-xs text-amber-800 dark:text-amber-200">
            {expired
              ? t("providerForm.chatgptSignInExpiredBody")
              : t("providerForm.chatgptSignInDeniedBody")}
          </span>
        )}
      ...
      >
        {phase.step === "starting"
          ...
          : signedIn
            ...
            : t("providerForm.chatgptSignInStart")}
      </Button>
      {!signedIn && (
        <span className="text-xs text-neutral-500 dark:text-neutral-400">
          {t("providerForm.chatgptSignInHint")}
        </span>
      )}
    </div>
  );

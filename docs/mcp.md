...
| `run(task, url?, background?, images?)` | Give TabRunner a task in plain language. Returns immediately with a run id — browser work takes minutes. Adopts the tab the user is on by default (or opens its own at `url`, or when there's no page to work); `background: false` keeps the panel open so the user watches. Optional images ride along as base64. A task submitted while another runs **queues** and answers with its position instead of an error. |
